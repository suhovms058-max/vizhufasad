...
export function createPartnerCreditPagesRouter({
  authService, partnerCreditService, siteOrigin,
}) {
  const router = express.Router();
  const requireHtmlSession = createRequireSession(authService, { html: true });
  const limiter = rateLimit({ windowMs: 60 * 60 * 1000, limit: 10, standardHeaders: "draft-8", legacyHeaders: false });
  router.post(
    "/app/partner-code/redeem",
    limiter,
    ...
    async (request, response, next) => {
      try {
        ...
        const result = await partnerCreditService.redeem(request.auth.user_id, request.body);
        return response.redirect(303, `/app/balance?partner_credits=${result.credits}`);
      } catch (error) {
        if (error?.code) return response.redirect(303, `/app/balance?partner_code_error=${encodeURIComponent(error.code)}`);
        return next(error);
      }
    },
  );

...
export default function PartnersPage() {
  return <main className="seoPage partnerPage">
    <header className="seoHeader shell">
      ...
      <nav aria-label="Основная навигация"><a href="/gallery">Примеры</a><a href="/styles">Стили</a><a href="/#pricing">Тарифы</a><a className="seoHeaderCta" href={mailto}>Обсудить договор</a></nav>
    </header>
    ...
    <section className="seoHero partnerHero">
      <div className="shell partnerHeroGrid">
        <div>
          ...
          <div className="partnerHeroActions"><a className="button lightButton" href={mailto}>Обсудить условия</a><a className="partnerTextLink" href="/documents/vizhufasad-partner-contract-template.pdf" target="_blank" rel="noopener">Посмотреть образец договора →</a></div>
        </div>
    ...
    <section className="partnerHow">
      <div className="shell"><p className="eyebrow light"><span /> КАК НАЧАТЬ</p><h2>От договора до рабочего кабинета</h2>
        <ol className="partnerSteps">
          ...
          <li><span>03</span><div><strong>Активируете код</strong><p>Партнёр входит в кабинет с указанным в договоре email и вводит код в разделе баланса. Другой аккаунт использовать его не сможет.</p></div></li>
    ...
    <section className="partnerTerms shell">
      ...
      <dl>
        ...
        <div><dt>Объём</dt><dd>Точное количество ВФ-коинов и срок, в течение которого можно активировать код и использовать услуги.</dd></div>
    ...
    <section className="partnerFaq shell"><p className="eyebrow"><span /> ВОПРОСЫ</p><h2>Перед заключением договора</h2><div>
      ...
      <details><summary>Открывает ли код потребительский тариф?</summary><p>Нет. Партнёрский код начисляет договорный номинал ВФ-коинов без покупки потребительского пакета. Доступные действия и их стоимость показываются в кабинете до запуска.</p></details>
    ...
    <section className="partnerFinal"><div className="shell"><p className="eyebrow light"><span /> ОБСУДИТЬ СОТРУДНИЧЕСТВО</p><h2>Согласуем объём под ваши задачи</h2><p>Напишите владельцу ВИЖУФАСАД. В письме укажите компанию, ИНН, контактное лицо и предполагаемое количество ВФ-коинов.</p><a className="button lightButton" href={mailto}>Написать владельцу</a><a className="partnerEmail" href={`mailto:${contactEmail}`}>{contactEmail}</a></div></section>

...
- Публичный лендинг и юридические страницы опубликованы на `https://46-149-67-190.sslip.io`; кабинет, API и генерация закрыты дополнительной HTTP-аутентификацией от случайного расходования GenAPI. Подписанные webhook Robokassa доступны без staging-пароля по отдельным точным маршрутам.
...
- Статический лендинг для nginx собирается командой `NEXT_OUTPUT=export npm run build`; обычный standalone build не создаёт обязательный каталог `out`.

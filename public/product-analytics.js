(() => {
  ...
  const showSettings = () => {
    ...
    banner.className = "privacy-banner"; banner.dataset.privacyBanner = "";
  ...
  document.addEventListener("click", (event) => { if (event.target.closest?.("[data-privacy-settings]")) showSettings(); });
  if (!choice || choice.version !== consentVersion) showSettings(); else if (analyticsAllowed()) track("page_view");
  document.addEventListener("click", (event) => {
    const target = event.target.closest?.("[data-analytics-event]");
    if (!target) return;
    track(target.dataset.analyticsEvent, { placement: target.dataset.analyticsPlacement || "", plan: target.dataset.analyticsPlan || "" });
  });
})();
